...
getScoreTable <- function(param, iterations, stepsave, MAP = TRUE, posterior = 0.5,
  ...
  ptab <- listpossibleparents.PC.aliases(startskel,
    isgraphNEL = FALSE,
    n, updatenodes
  )
  ...
  # aliases for each node since all nodes in parent tables are
  # done as 1,2,3,4... not real parent names
  ...
  scoretable <- scorepossibleparents.PLUS1(
    parenttable, plus1lists, n,
    param, updatenodes,
    rowmaps, numparents,
    numberofparentsvec
  )
...
orderMCMCFelix <- function(scorepar, MAP = TRUE, plus1 = TRUE, chainout = FALSE, scoreout = FALSE, moveprobs = NULL,
  ...
  result <- orderMCMCmainFelix(
    param = scorepar, iterations, stepsave, startorder = startorder,
...
orderMCMCmainFelix <- function(param, iterations, stepsave, MAP = TRUE, posterior = 0.5,
  ...
  if (MAP == TRUE) {
    ...
    bannedscore <- poset.scores(posetparenttable, scoretable, ptab$numberofparentsvec, rowmaps,
      n,
      plus1lists = plus1lists, ptab$numparents, updatenodes
    )
  }
  ##############################
