...
orderscorePlus1 <- function(n, scorenodes, scorepositions, parenttable, aliases, numparents,
  ...
  for (i in scorenodes) {
    ...
    if (position == n) {
      ...
      therows[i] <- c(2 ^ numparents[i]) ## all parents are banned / Felix. \bar f(Z) = 2^0 + 2^1 + 2^2 + 1?  +1 for r indexing?
    } else {
      bannednodes <- permy[1:position] # I dont understand the meaning of this! # Why care aboput these elements?
      allowednodes <- permy[(position + 1):n]
      # aliases are the possible parents?
      # But it they are found in bannednodes, they are regarded as banned? Arent there other ways of banning, id by restricting the space? 

      # Is aliases Z? Arent bannednodes already banned? /Felix
      # aliases is h = {h1,h2,h3,...,hK}, the possible parents?
      bannedpool <- which(aliases[[i]] %in% bannednodes) #bannedpool hols the hj's or in particular the j's.
      # bannedpool is "h \in <" = {h1,h2,h3,...,hK}, the possible parent sets permitted by the order.
      if (numparents[i] == 0 || length(bannedpool) == 0) {
        # all parents allowed
        therows[i] <- c(1) # is therows \bar f(Z) + 1? 
      } else {
        therows[i] <- rowmaps[[i]]$backwards[sum(2 ^ bannedpool) / 2 + 1] # What is rowmaps? Is it really needed? Seems to be the identity map.
      }
...
positionscorePlus1 <- function(n, nsmall, currentscore, positionx, permy, aliases, rowmaps, plus1lists, numparents,
  ...
  if (positionx < nsmall) {
    ...
    for (i in (positionx + 1):nsmall) {
      ...
      if (numparents[x] == 0) {
        #there is only 1 row in the score table
        therowx[i] <- c(1)
      } else if (i == n) {
        #no parents allowed, i.e. only first row, only first list
